...
quit_quiz.onclick = () => {
  location.replace("./start.html"); //reload the current window
};
...
leaderBoard.onclick = async () => {
  const resultCard = document.querySelector(".result_box"); // Replace with the selector of your scorecard element
  const resultText = `I scored ${userScore} out of ${
    questions.length - 20
  } on the quiz! Can you beat my score?`;

  // Use html2canvas to capture the scorecard as an image
  try {
    const canvas = await html2canvas(resultCard, { useCORS: true }); // Ensure cross-origin images are handled
    ...
    if (navigator.share && navigator.canShare({ files: [new File([blob], "quiz-score.png", { type: "image/png" })] })) {
      ...
      // Fallback for sharing as image URL
      ...
      `;
      document.querySelector(".social-share").innerHTML = shareMessage; // Update the DOM with sharing options
    }
  } catch (error) {
